...
module PSRAM_READER (
    ...
    localparam  IDLE = 1'b0,
                READ = 1'b1;
    reg qpi_flag;
    wire [7:0]  FINAL_COUNT = qpi_flag?7:(13 + size*2); // was 27: Always read 1 word
    ...
    always @*
        case (state)
            IDLE: if(rd|qpi_flag) nstate = READ; else nstate = IDLE;
    ...
    always @ (posedge clk or negedge rst_n)
        if(!rst_n) qpi_flag<=1'b1 ;
        else if(counter==8)qpi_flag<=1'b0;
    ...
    assign dout     =   ((counter < 8)&&qpi_flag)   ?   {3'b0, CMD_35H[7 - counter]}:
